...
//    }
    ...
    func zipAllMyFiles(_ newZipFile:String , vc:UIViewController, files : [FBFile], canShare:Bool) -> Bool {
    ...
    func shareMultipleZipFile(_ filesPath:[AnyObject], vc:UIViewController) -> Void {
    ...
    func shareAllMyFile(_ vc:FileListViewController, files : [FBFile] ) -> Void {
    ...
    func showAlertToCombineZipFiles(_ name:String, vc:FileListViewController, files : [FBFile]){
        ...
        let saveAction = UIAlertAction(title: "NO", style: UIAlertActionStyle.default, handler: {
            ...
            var zipFiles = [URL]()
            for file in files {
                zipFiles.append(file.filePath as URL)
            }
            let myWebsite = URL(string: "https://itunes.apple.com/us/app/smartzip/id1141913794?ls=1&mt=8")
            zipFiles.append(myWebsite!)
            self.shareMultipleZipFile(zipFiles as [AnyObject], vc: vc)
    ...
    func isAllZipFiles(_ files : [FBFile]) -> Bool {
    ...
    func containsImageOrVideoFile(_ files : [FBFile]) -> Bool {
